<div align="center">

  <!-- Compact Dynamic Header -->
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=0,2,10,20,30&height=180&section=header&text=Deeksha%20G&fontSize=50&fontAlignY=40&desc=Software%20Engineer%20%7C%20AI%20%26%20Cloud%20Architect&descSize=18&descAlignY=64&fontColor=ffffff" width="100%" alt="Header" />

  <!-- Animated Typing Tagline -->
  <a href="https://github.com/DeekshaG96">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=18&duration=2500&pause=1000&color=38BDF8&center=true&vCenter=true&width=650&height=45&lines=%F0%9F%9A%80+Engineering+0-to-1+AI-native+platforms;%F0%9F%8C%90+Open+Source+Contributor+%40+GirlScript+Summer+of+Code+(GSSoC);%F0%9F%A7%A0+Computer+Science+%26+Business+Systems+%40+SIT+(2027);%F0%9F%92%BC+Ex-Intern+%40+Microsoft+%26+SAP+(AI)+%7C+AWS+Academy+%7C+Fortinet" alt="Typing SVG" />
  </a>

  <br>

  <!-- Connect Links -->
  <p align="center">
    <a href="https://linkedin.com/in/deeksha-g-cybersec" target="_blank">
      <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
    </a>
    &nbsp;
    <a href="https://iitsmedeeksha.netlify.app" target="_blank">
      <img src="https://img.shields.io/badge/Portfolio-00C7B7?style=for-the-badge&logo=safari&logoColor=white" alt="Portfolio" />
    </a>
    &nbsp;
    <a href="mailto:deekshagpbangera@gmail.com">
      <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
    </a>
  </p>

</div>

---

### 👩‍💻 About Me

- 🎓 **Undergraduate:** B.E. in **Computer Science & Business Systems** at **Srinivas Institute of Technology (SIT), Mangaluru** (Expected June 2027) | **CGPA: 7.8 / 10.0**.
- 💼 **Experience:**
  - 🌐 Open Source Contributor @ **GirlScript Summer of Code (GSSoC)** (May 2026 – Present).
  - 🤖 AI & Data Intern @ **Microsoft & SAP** (94% accuracy on ML models across 10k+ records).
  - ☁️ Cloud Infrastructure Intern @ **AWS Academy** (Grade A) & Security Intern @ **Fortinet** (Grade O).
- 🚀 **Focus:** Building 0-to-1 full-stack platforms, Generative AI workflows, and resilient cloud architectures.

---

### 🛠️ Tech Stack

<div align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=python,java,js,ts,react,nextjs,nodejs,tailwind,aws,mysql,mongodb,git,postman,docker&theme=dark" alt="Skills" />
  </a>
</div>

---

### 🚀 Featured 0-to-1 Projects

- 🌾 **[Smart Farmer Connect](https://github.com/DeekshaG96)** — Agritech analytics web app built with Next.js, Tailwind CSS & Vercel (**+40% user engagement**).
- 💳 **[RazorOps AI](https://github.com/DeekshaG96)** — FinTech operations portal & automated SQL reconciliation engine (**-40% manual audit overhead**).
- 🚚 **[Eco-Track Logistics](https://github.com/DeekshaG96)** — AI-native routing service integrating IoT sensor telemetry with Gemini LLM reasoning (**+25% delivery efficiency**).

---

### 📊 GitHub Activity

<div align="center">
  <img src="https://github-readme-stats.anuraghazra1.vercel.app/api?username=DeekshaG96&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" width="48.5%" alt="GitHub Stats" />
  <img src="https://github-readme-stats.anuraghazra1.vercel.app/api/top-langs/?username=DeekshaG96&layout=compact&theme=tokyonight&hide_border=true" width="48.5%" alt="Top Languages" />
</div>

<br>

---

<div align="center">
  <sub>Crafted with passion & precision by <strong>Deeksha G</strong> • Connect on <a href="https://linkedin.com/in/deeksha-g-cybersec">LinkedIn</a> or explore my <a href="https://iitsmedeeksha.netlify.app">Portfolio</a>.</sub>
</div>
